...
import { generateOrders } from '../utils/mock-data-generator'
...
export class MockDataService {
  ...
  private initializeMockData() {
    logger.info('初始化模拟数据...')
    this.orders = generateOrders(1000) // 生成1000条模拟订单
    logger.info(`已生成 ${this.orders.length} 条模拟订单数据`)
  }
  
  /**
   * 根据查询参数筛选模拟订单数据
   */
  async queryOrders(params: any) {
    const {
      startTime,
      endTime,
      stationCodes,
    ...
    // 应用门店筛选
    if (stationCodes) {
      const stationArray = stationCodes.split(',').map((code: string) => code.trim())
      filteredData = filteredData.filter(order => 
        stationArray.includes(order.所属门店代码)
      )
    }
    ...
    if (status) {
      ...
      filteredData = filteredData.filter(order => 
        statusArray.includes(order.订单状态)
      )
    }
   ...
   * 获取订单统计信息
   */
  async getOrderStats(params: any) {
    ...
    // 按门店统计
    const storeDistribution: Record<string, number> = {}
   ...
   * 触发导出任务
   */
  async createExportTask(params: any) {
   ...
   * 模拟数据库查询延迟
   */
  private simulateQueryDelay(maxDelay = 300): Promise<void> {
