...
exports.convertToOrderRecord = convertToOrderRecord;
exports.convertToOrderRecords = convertToOrderRecords;
exports.getOrderStatusText = getOrderStatusText;
exports.getOrderTypeText = getOrderTypeText;
exports.getSourceChannelText = getSourceChannelText;
exports.getDeliveryTypeText = getDeliveryTypeText;
exports.getPaymentTypeText = getPaymentTypeText;
exports.validateOrderRecord = validateOrderRecord;
/**
 ...
 * @returns 格式化后的订单记录
 */
function convertToOrderRecord(rawRow) {
    // 确保原始数据存在
    if (!rawRow) {
 ...
 * @returns 格式化后的订单记录数组
 */
function convertToOrderRecords(rawRows) {
 ...
 * 创建空的订单记录（用于错误处理）
 */
function createEmptyOrderRecord(orderNumber) {
    return {
        订单号: orderNumber,
        来源渠道: '',
        下单人手机号: '',
...
function getOrderStatusText(statusCode) {
    ...
    return types_1.OrderStatus[code] || '未知状态';
}
/**
 * 将数字类型的订单类型转换为中文描述
 */
function getOrderTypeText(typeCode) {
    ...
    return types_1.OrderType[code] || '未知类型';
}
/**
 * 将数字类型的来源渠道转换为中文描述
 */
function getSourceChannelText(channelCode) {
    const code = String(channelCode);
    return types_1.OrderSourceChannel[code] || '未知渠道';
}
/**
 * 将数字类型的配送方式转换为中文描述
 */
function getDeliveryTypeText(deliveryCode) {
    const code = String(deliveryCode);
    return types_1.DeliveryType[code] || '未知方式';
}
/**
 * 将支付类型代码转换为中文描述
 */
function getPaymentTypeText(paymentCode) {
    return PaymentType[paymentCode] || '未知支付';
}
/**
 * 验证订单记录是否有效（基本验证）
 */
