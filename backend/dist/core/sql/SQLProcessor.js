...
exports.SQLProcessor = void 0;
const tslib_1 = require("tslib");
const promises_1 = tslib_1.__importDefault(require("fs/promises"));
const path_1 = tslib_1.__importDefault(require("path"));
const logger_1 = require("../../utils/logger");
const dataConverters_1 = require("../../utils/dataConverters");
// SQL处理器类
class SQLProcessor {
    templatesPath;
    templates = new Map();
    ...
    async loadTemplate(templateName) {
        try {
            const templatePath = path_1.default.join(this.templatesPath, `${templateName}.sql`);
            const sqlContent = await promises_1.default.readFile(templatePath, 'utf-8');
            // 分析SQL模板，提取参数信息和分页配置
            ...
            logger_1.logger.debug(`已加载SQL模板: ${templateName}`, { paramCount: parameters.length });
            return template;
        }
        catch (error) {
            logger_1.logger.error(`加载SQL模板失败: ${templateName}`, error);
            throw new Error(`无法加载SQL模板: ${templateName}`);
        }
    }
    // 处理查询参数（兼容新老接口）
    processQuery(templateName, queryParams) {
    ...
    // 构建订单查询参数
    buildOrderQueryParams(queryParams) {
        const params = [];
        // 按照SQL模板中的参数顺序构建
        // 1. 开始时间
    ...
    // 添加参数化方法
    parameterizeTemplate(sql, filters) {
        // 替换时间参数占位符
    ...
    buildParameterValues(originalSQL, filters) {
        const params = [];
        if (!filters)
            return params;
        // 按照SQL中参数的顺序构建值数组
    ...
    clearTemplates() {
        ...
        logger_1.logger.debug('已清空SQL模板缓存');
    }
    // 数据处理方法
    processQueryResults(rawData, queryParams) {
        if (!rawData || !Array.isArray(rawData)) {
            return { records: [] };
        }
        try {
            // 转换为订单记录
            const records = (0, dataConverters_1.convertToOrderRecords)(rawData);
            // 可选：计算总数（如果有的话）
            ...
            logger_1.logger.error('处理查询结果失败:', error);
            throw error;
        }
    }
    // 处理查询统计信息（用于分页）
    processCountingSQL(queryParams) {
        const templateName = 'order-reconciliation';
        const template = this.templates.get(templateName);
        if (!template) {
            throw new Error(`SQL模板未找到: ${templateName}`);
...
// 处理助力活动查询参数
processSupportQuery(queryParams, {
    startTime: string,
    endTime: string,
    activityId: string,
    page: number,
...
ProcessedSQL;
{
    const templateName = 'support-activity-query';
    const template = this.templates.get(templateName);
    if (!template) {
...
// 处理商品渗透率查询参数
processPenetrationQuery(queryParams, {
    startTime: string,
    endTime: string,
    stationCodes: string,
    barCode: string,
...
ProcessedSQL;
{
    const templateName = 'penetration-query';
    const template = this.templates.get(templateName);
    if (!template) {
        throw new Error(`SQL 模板未找到：${templateName}`);
    }
    ...
    const offset = (page - 1) * pageSize;
    // 构建参数（按 SQL 模板顺序）
    params.push(queryParams.startTime, queryParams.endTime); // 子查询 1
    params.push(queryParams.startTime, queryParams.endTime); // 子查询 2
    params.push(queryParams.startTime, queryParams.endTime); // 子查询 3
    params.push(queryParams.startTime, queryParams.endTime); // 子查询 4
    params.push(queryParams.startTime, queryParams.endTime); // WHERE 条件
    params.push(queryParams.barCode || '', queryParams.barCode || '');
    params.push(queryParams.partyCode || '', queryParams.partyCode || '');
    params.push(queryParams.stationCodes || '', queryParams.stationCodes || '');
    params.push(pageSize, offset);
    return { sql, params, paramCount: params.length, hasPagination: template.hasPagination };
}
// 处理优惠券查询参数
processCouponQuery(queryParams, {
...
ProcessedSQL;
{
    const templateName = 'coupon-query';
    const template = this.templates.get(templateName);
    if (!template) {
        throw new Error(`SQL 模板未找到：${templateName}`);
    }
    ...
    const pageSize = queryParams.pageSize || 20;
    const offset = (page - 1) * pageSize;
    params.push(queryParams.receiveStartTime || '', queryParams.receiveStartTime || '');
    params.push(queryParams.receiveEndTime || '', queryParams.receiveEndTime || '');
    params.push(queryParams.useStartTime || '', queryParams.useStartTime || '');
    params.push(queryParams.useEndTime || '', queryParams.useEndTime || '');
    params.push(queryParams.couponIds || '', queryParams.couponIds || '');
    params.push(pageSize, offset);
    return { sql, params, paramCount: params.length, hasPagination: template.hasPagination };
}
// 处理社群拉新查询参数
processInvitationQuery(queryParams, {
    startTime: string,
    endTime: string,
    page: number,
    pageSize: number
...
ProcessedSQL;
{
    const templateName = 'invitation-query';
    const template = this.templates.get(templateName);
    if (!template) {
        throw new Error(`SQL 模板未找到：${templateName}`);
    }
    ...
    const pageSize = queryParams.pageSize || 20;
    const offset = (page - 1) * pageSize;
    params.push(queryParams.startTime, queryParams.endTime);
    params.push(pageSize, offset);
    return { sql, params, paramCount: params.length, hasPagination: template.hasPagination };
}
// 处理商城用户查询参数
processMallUserQuery(queryParams, {
    date: string,
    mobile: string,
    page: number,
    pageSize: number
...
ProcessedSQL;
{
    const templateName = 'mall-user-query';
    const template = this.templates.get(templateName);
    if (!template) {
        throw new Error(`SQL 模板未找到：${templateName}`);
    }
    ...
    params.push(queryParams.mobile || '', queryParams.mobile || '');
    params.push(pageSize, offset);
    return { sql, params, paramCount: params.length, hasPagination: template.hasPagination };
}
//# sourceMappingURL=SQLProcessor.js.map